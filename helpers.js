...
async function audit(req, actorRole, action, entityType, entityId, details) {
  ...
  const locId = getSession(req)?.locationId || 1;
  await run(
    `INSERT INTO audit(at,actorRole,action,entityType,entityId,details,ip,userAgent,location_id)
     VALUES(?,?,?,?,?,?,?,?,?)`,
    [Date.now(), actorRole || 'unknown', action, entityType, entityId, d,
     ipOf(req), req.headers['user-agent'] || '', locId]
  );
}
