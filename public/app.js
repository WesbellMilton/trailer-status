(() => {
  ...
  el("modalOv").addEventListener("click", e => {
    if (e.target === el("modalOv")) { el("modalOv").classList.add("hidden"); if(_mr){_mr(false);_mr=null;} }
  });

  function setPlatesOpen(open) {
    const t = el("dockPlatesToggle"), b = el("dockPlatesBody"); if (!t || !b) return;
    t.setAttribute("aria-expanded", open ? "true" : "false");
    if (!open) { b.style.maxHeight = "0px"; try{localStorage.setItem("platesOpen","0");}catch{} return; }
    requestAnimationFrame(() => { b.style.maxHeight = (b.scrollHeight+40)+"px"; });
    try{localStorage.setItem("platesOpen","1");}catch{}
  }
  ...
  function statusTag(s) {
    const cls = STATUS_TAG[s] || "stag-unknown";
    return `<span class="stag ${cls}"><span class="sp"></span>${esc(s||"—")}</span>`;
  }
  ...
  function highlightNav() {
    ...
    if (ROLE) { rb.style.display = ""; rb.textContent = ROLE.toUpperCase(); }
  ...
  function renderBoardInto(tbodyEl, countEl, countStrEl, sq, dq, stq, readOnly) {
    ...
    tbodyEl.innerHTML = filt.map(r => {
      ...
      if (canEdit) {
        ...
        if (r.status === "Dock Ready") {
          acts = `<div class="t-acts">${sel}<button class="btn btn-success btn-sm" data-act="markReady" data-trailer-id="${esc(r.trailer)}" style="font-weight:800;">✓ Mark Ready</button><button class="btn btn-default btn-sm" data-act="edit" data-trailer-id="${esc(r.trailer)}">Edit</button><button class="btn btn-danger btn-sm" data-act="delete" data-trailer-id="${esc(r.trailer)}">Del</button></div>`;
        } else {
          acts = `<div class="t-acts">${sel}<button class="btn btn-default btn-sm" data-act="edit" data-trailer-id="${esc(r.trailer)}">Edit</button><button class="btn btn-danger btn-sm" data-act="delete" data-trailer-id="${esc(r.trailer)}">Del</button></div>`;
        }
      } else if (canDock) {
  ...
  function renderPlates() {
    ...
    if (pm) {
      ...
      pm.textContent = `${v.filter(p=>p?.status==="OK").length} OK · ${v.filter(p=>p?.status==="Service").length} Svc`;
    }
    el("platesGrid").innerHTML = doors.map(door => {
      ...
      return `<div class="plate ${cls}">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:3px;">
          <span class="p-door">D${esc(door)}</span>${plateStatusTag(p.status)}
        </div>
  ...
  function renderConf() {
    ...
    el("confBody").innerHTML = !confirmations.length
      ? `<tr><td colspan="4" style="padding:16px;color:var(--t2);">No safety confirmations yet.</td></tr>`
      : confirmations.map(c=>`<tr><td class="muted">${esc(fmtTime(c.at))}</td><td class="mono" style="font-weight:500;color:var(--t0);">${esc(c.trailer||"—")}</td><td class="mono">${esc(c.door||"—")}</td><td class="muted">${esc((c.ip||"—").split(",")[0])}</td></tr>`).join("");
  }
  function renderSupConf() {
    ...
    sb.innerHTML = !confirmations.length
      ? `<tr><td colspan="4" style="padding:16px;color:var(--t2);">No confirmations yet.</td></tr>`
      : confirmations.map(c=>`<tr><td class="muted">${esc(fmtTime(c.at))}</td><td class="mono" style="font-weight:500;color:var(--t0);">${esc(c.trailer||"—")}</td><td class="mono">${esc(c.door||"—")}</td><td class="muted">${esc((c.ip||"—").split(",")[0])}</td></tr>`).join("");
  }

  const FEED_COLORS = {trailer_create:"var(--green)",trailer_update:"var(--cyan)",trailer_delete:"var(--red)",trailer_status_set:"var(--amber)",driver_drop:"var(--violet)",safety_confirmed:"var(--green)",plate_set:"var(--t2)",pin_changed:"var(--amber)",trailer_clear_all:"var(--red)"};
  ...
  function dispPanelHtml() { return `
    <div class="infobox infobox-amber">
      ...
      Add and manage trailers. When dock marks <strong>Dock Ready</strong>, hit <strong>✓ Mark Ready</strong> on that row.
    </div>
  ...
  function renderRolePanel() {
    const dpc = el("dockPlatesCard");
    if (ROLE === "dispatcher") {
      el("panelTitle").textContent = "Dispatcher"; el("panelSub").textContent = "Full control";
      el("panelBody").innerHTML = dispPanelHtml();
      el("btnLogout").style.display = ""; el("btnAudit").style.display = "";
      if (dpc) dpc.style.display = ""; renderPlates(); return;
    }
    if (ROLE === "dock") {
      el("panelTitle").textContent = "Dock"; el("panelSub").textContent = "Loading / Dock Ready";
  ...
  async function markReady(trailer) {
    try { await apiJson("/api/upsert",{method:"POST",headers:CSRF,body:JSON.stringify({trailer,status:"Ready"})}); toast("Trailer Ready",`${trailer} marked Ready — driver will be notified.`,"ok"); }
    catch(e) { toast("Update failed",e.message,"err"); }
  }
  async function plateSave(door) {
    ...
    try {
      ...
      plateEditOpen[door] = false; renderPlates();
    } catch(e) { toast("Update failed",e.message,"err"); }
  }
  async function setPin(role, inputId, confirmId) {
    ...
    try {
      ...
      el(inputId).value = ""; el(confirmId).value = "";
    } catch(e) { toast("Update failed",e.message,"err"); }
  }
  ...
  ───────────────────────────────────────────────── */
  const driverState = {
    step: 1,           // 1 = drop, 2 = safety, 3 = done
    trailer: "",
    assignedDoor: "",  // from dispatcher lookup
    selectedDoor: "",  // final chosen door (assigned or manual)
    dropType: "Empty",
    overrideMode: false,
    sessionDrops: [],  // persisted to sessionStorage
  };

  // Load session history from sessionStorage
  try {
    const saved = sessionStorage.getItem("wb_driver_session");
    if (saved) driverState.sessionDrops = JSON.parse(saved);
  } catch {}

  function saveSessionHistory() {
    try { sessionStorage.setItem("wb_driver_session", JSON.stringify(driverState.sessionDrops)); } catch {}
  }

  function renderSessionHistory() {
    const count = el("shCount"), body = el("shBody");
    if (!count || !body) return;
    const drops = driverState.sessionDrops;
    count.textContent = `${drops.length} drop${drops.length === 1 ? "" : "s"}`;
    if (!drops.length) {
      body.innerHTML = `<div class="sh-empty">No drops submitted yet this session.</div>`;
      return;
    }
    body.innerHTML = drops.slice().reverse().map(d => `
      <div class="sh-row">
        <div>
          ...
          <span class="sh-meta"> · D${esc(d.door)} · ${esc(d.dropType)}</span>
        </div>
        <div>
          <span class="sh-type ${d.safetyDone ? 'sh-status-ok' : 'sh-status-err'}">${d.safetyDone ? "✓ Safety" : "⚠ No safety"}</span>
  ...
  function buildDoorPicker() {
    const grid = el("doorPickerGrid"); if (!grid) return;
    const occupiedDoors = new Set(
      Object.values(trailers)
        .filter(r => r.door && !["Departed",""].includes(r.status))
        .map(r => r.door)
    );
    let html = "";
    for (let d = 18; d <= 42; d++) {
      const ds = String(d);
      const occupied = occupiedDoors.has(ds);
      const selected = driverState.selectedDoor === ds && !driverState.assignedDoor;
      html += `<button class="door-btn${occupied?" occupied":""}${selected?" selected":""}" data-door="${ds}">
        ${ds}
        ${occupied ? `<span class="door-btn-sub">In use</span>` : ""}
      </button>`;
    }
    grid.innerHTML = html;
  }
  ...
  function showDoorPicker() {
    buildDoorPicker();
    el("doorPickerWrap").classList.add("visible");
    el("assignmentCard").classList.remove("visible");
  }
  function hideDoorPicker() {
    el("doorPickerWrap").classList.remove("visible");
  }
  function showAssignmentCard(door, meta) {
    el("ac_door").textContent = "Door " + door;
    el("ac_meta").textContent = meta || "Assigned by dispatcher";
    el("assignmentCard").classList.add("visible");
    hideDoorPicker();
  }

  function updateDriverSubmitState() {
    const btn = el("btnDriverDrop"); if (!btn) return;
    const hasTrailer = driverState.trailer.trim().length > 0;
    const hasDoor = driverState.selectedDoor.length > 0;
    btn.disabled = !(hasTrailer && hasDoor);
  }

  function setDriverStep(step) {
    driverState.step = step;

    // Update progress nodes
    [1,2,3].forEach(i => {
      const node   = el(`stepNode${i}`);
      const circle = el(`stepCircle${i}`);
      if (!node || !circle) return;
      node.classList.remove("active","done");
      circle.classList.remove("active","done");
      if (i < step) { node.classList.add("done"); circle.classList.add("done"); circle.textContent = "✓"; }
      else if (i === step) { node.classList.add("active"); circle.classList.add("active"); circle.textContent = i === 3 ? "✓" : i; }
      else { circle.textContent = i === 3 ? "✓" : i; }
    });

    // Connectors
    [1,2].forEach(i => {
      const conn = el(`stepConn${i}`);
      if (conn) conn.classList.toggle("done", step > i);
    });

    const s1 = el("driverStep1"), s2 = el("driverStep2"), done = el("driverDone");
    if (step === 1) {
      s1.style.display = ""; s1.classList.add("active-step"); s1.style.opacity = "1"; s1.style.pointerEvents = "";
      s2.style.display = ""; s2.classList.remove("active-step"); s2.style.opacity = ".5"; s2.style.pointerEvents = "none";
      done.style.display = "none";
    } else if (step === 2) {
      s1.style.display = ""; s1.classList.remove("active-step"); s1.style.opacity = ".6"; s1.style.pointerEvents = "none";
      s2.style.display = ""; s2.classList.add("active-step"); s2.style.opacity = "1"; s2.style.pointerEvents = "";
      done.style.display = "none";
      // Pre-fill safety context
      const ctx = el("safetyContext");
      if (ctx) {
        ctx.innerHTML = [
          driverState.trailer ? `<span class="context-chip">🚛 Trailer <strong>${esc(driverState.trailer)}</strong></span>` : "",
          driverState.selectedDoor ? `<span class="context-chip">🚪 Door <strong>${esc(driverState.selectedDoor)}</strong></span>` : "",
          `<span class="context-chip">📦 <strong>${esc(driverState.dropType)}</strong></span>`,
        ].join("");
      }
    } else if (step === 3) {
      s1.style.display = "none"; s2.style.display = "none";
      done.style.display = "";
      el("driverDoneDetail").innerHTML =
        `Trailer <strong>${esc(driverState.trailer)}</strong> · Door <strong>${esc(driverState.selectedDoor)}</strong> · ${esc(driverState.dropType)}<br>Drop + safety recorded.`;
    }
  }

  /* Debounced trailer lookup */
  ...
  function onTrailerInput() {
    ...
    // Reset door state when trailer changes
    ...
    _lookupTimer = setTimeout(() => lookupTrailerAssignment(val), 500);
  }

  async function lookupTrailerAssignment(trailer) {
    const spinner = el("lookupSpinner");
    if (spinner) spinner.classList.add("visible");
    try {
      const res = await fetch(`/api/driver/assignment?trailer=${encodeURIComponent(trailer)}`, {
      ...
      if (data.found && data.door) {
        ...
        driverState.overrideMode = false;
        const metaParts = [];
        if (data.direction) metaParts.push(data.direction);
        if (data.status) metaParts.push(data.status);
        showAssignmentCard(data.door, metaParts.join(" · ") || "Assigned by dispatcher");
      } else {
        driverState.assignedDoor = "";
        if (!driverState.overrideMode) {
          driverState.selectedDoor = "";
      ...
      // On lookup failure just show manual picker
      if (!driverState.overrideMode) showDoorPicker();
    } finally {
  ...
  async function driverDrop() {
    const trailer = driverState.trailer;
    const door    = driverState.selectedDoor;
    const dropType = driverState.dropType;
    ...
    try {
      ...
      // Add to session history (no safety yet)
      driverState.sessionDrops.push({trailer, door, dropType, at: Date.now(), safetyDone: false});
      saveSessionHistory();
      renderSessionHistory();
      // Advance to safety step
      setDriverStep(2);
  ...
  function updateSafetySubmitState() {
    const btn = el("btnConfirmSafety"); if (!btn) return;
    const ok = el("c_loadSecured")?.checked && el("c_dockPlateUp")?.checked;
    btn.disabled = !ok;
  }

  async function confSafety() {
    if (!el("c_loadSecured")?.checked || !el("c_dockPlateUp")?.checked)
      return toast("Incomplete","Both safety items must be confirmed.","err");
    try {
      await apiJson("/api/confirm-safety",{method:"POST",headers:CSRF,body:JSON.stringify({
        trailer: driverState.trailer,
        door:    driverState.selectedDoor,
        loadSecured: true,
        dockPlateUp: true,
      })});
      // Mark last session entry as safety done
  ...
  /* Driver WS dot mirrors main dot */
  function syncDriverWsDot(state) {
    const dot = el("driverWsDot"), txt = el("driverWsText");
    if (!dot || !txt) return;
  ...
  async function loadInitial() {
    ...
    if (isSuper() || ROLE === "supervisor") {
      renderSupBoard(); renderSupConf();
      loadAuditInto(null, el("supAuditCount"), 0);
    } else if (isDriver()) {
      // Build door picker with live occupancy data
      buildDoorPicker();
    } else {
      renderRolePanel(); renderBoard(); renderConf();
      let open = false; try { open = localStorage.getItem("platesOpen") === "1"; } catch {}
      setPlatesOpen(open);
    }
  }
  ...
  document.addEventListener("click", async ev => {
    const t = ev.target.closest
      ? (ev.target.closest("[data-door]") || ev.target.closest("[data-type]") || ev.target.closest("[data-act]") || ev.target.closest("[data-plate-toggle]") || ev.target.closest("[data-plate-save]") || ev.target) 
      : ev.target;
    ...
    if (id === "btnClearFilters" || id === "btnSupClearFilters") {
      ["search","filterDir","filterStatus","supSearch","supFilterDir","supFilterStatus"].forEach(i=>{if(el(i))el(i).value="";});
      renderBoard(); renderSupBoard(); return;
    }
    if (id === "btnAudit") {
      const showing = el("auditCard").style.display !== "none";
      el("auditCard").style.display = showing ? "none" : "";
      if (!showing) loadAuditInto(el("auditBody"), el("auditCount"), 7); return;
    }
    ...
    if (doorBtn && doorBtn.dataset.door) {
      ...
      driverState.overrideMode = true;
      buildDoorPicker(); // re-render to show selection
      updateDriverSubmitState();
      return;
    }
    ...
    if (act === "edit") {
      const tr = direct.dataset.trailerId, r = trailers[tr]; if (!r) return;
      el("d_trailer").value = tr; el("d_direction").value = r.direction || "Inbound";
      el("d_status").value = r.status || "Incoming"; el("d_door").value = r.door || "";
      el("d_note").value = r.note || ""; el("d_dropType").value = r.dropType || "";
      toast("Record loaded",`Editing trailer ${tr}`,"ok"); return;
    }
  ...
  // Auto-save dispatcher status dropdown
  document.addEventListener("change", ev => {
    const t = ev.target;
    if (t?.dataset?.act === "rowStatus") {
      const trailer = t.dataset.trailerId, status = t.value;
      apiJson("/api/upsert",{method:"POST",headers:CSRF,body:JSON.stringify({trailer,status})}).catch(e=>toast("Update failed",e.message,"err"));
    }
    // Safety checkbox state
    if (t?.id === "c_loadSecured" || t?.id === "c_dockPlateUp") updateSafetySubmitState();
  });

  // Driver trailer input
  ...
  function connectWs() {
    ...
    ws.onmessage = evt => {
      ...
      if (type === "state") {
        ...
        if (isDriver()) buildDoorPicker(); // refresh occupancy in picker
      }
        ...
        toast("Trailer Ready",`${payload.trailer} is READY${payload.door?" at door "+payload.door:""}.`,"ok");
    };
  }
