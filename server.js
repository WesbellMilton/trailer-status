...
process.on("uncaughtException", err => {
  ...
  logEvent("error", "crash", "uncaughtException", String(err?.stack || err)).catch(() => {});
});
process.on("unhandledRejection", reason => {
  console.error("[CRASH] unhandledRejection:", reason);
  logEvent("error", "crash", "unhandledRejection", String(reason?.stack || reason)).catch(() => {});
});

/* ══════════════════════════════════════════
   GZIP JSON (ONLY WHEN WORTH IT)
══════════════════════════════════════════ */
...
const APP_VERSION     = process.env.APP_VERSION || "3.5.1";
const PIN_MIN_LEN     = 4;
...
// Store VAPID keys on same disk as DB by default (Render-friendly)
const VAPID_FILE = process.env.VAPID_FILE || path.join(path.dirname(DB_FILE), "vapid.json");
...
async function loadDoorBlocksObject() {
  ...
  const o = {};
  rows.forEach(r => o[r.door] = { note: r.note, setAt: r.setAt });
  return o;
}
...
function loadOrGenVapid() {
  try {
    if (fs.existsSync(VAPID_FILE)) {
      ...
      console.log("[VAPID] Loaded existing keys:", VAPID_FILE);
      return;
    }
  } catch {}
  ...
  try {
    ...
    // Extract raw EC point (public) and private scalar from DER
    ...
    console.log("[VAPID] Generated new key pair:", VAPID_FILE);
  } catch (e) {
...
async function initDb() {
  ...
  // Cleanup / seed dock plates 28–42
  await run(`DELETE FROM dockplates WHERE CAST(door AS INTEGER) < 28`).catch(() => {});
  ...
  // Seed PINs — env vars always win so deploy config applies immediately
...
// Driver endpoints — accessible without login.
// But if a session exists, it must NOT be dock role (dock shouldn't submit driver actions).
...
// Issue reports can come from anyone (drivers or authenticated roles)
function requireIssueAccess(_req, _res, next) { next(); }

// Dock workers can only set Loading/Dock Ready via API (hard stop)
function requireDockStatusAllowed(req, res, next) {
  ...
  if (s.role === "dock") {
    ...
    const DOCK_ALLOWED = ["Loading", "Dock Ready"];
...
function actorOf(req) {
  return req.user?.role || getSession(req)?.role || "unknown";
}
...
async function pickBestDoor(excludeTrailer = null) {
  ...
  for (let d = 28; d <= 42; d++) {
    ...
    candidates.push({
      door: ds,
      priority: plateStatus === "OK" ? 0 : plateStatus === "Unknown" ? 1 : 2
    });
  }
...
async function releaseReservation(trailer) {
  const t = normTrailer(trailer);
  if (!t) return;
  await run(`DELETE FROM door_reservations WHERE trailer=?`, [t]).catch(() => {});
}
...
setInterval(cleanupExpiredReservations, 2 * 60 * 1000).unref();
...
const sendIndex = (_req, res) => {
  try {
    ...
    html = html.replace(
      "</head>",
      '<style>body,body::before,body::after{background-image:none!important;background:var(--bg)!important}#dispatchView,#dockView,#managementView,#driverView{background-image:none!important}</style></head>'
    );
...
function guardPage(allowedRoles) {
  return (req, res, next) => {
    ...
    if (role === "dispatcher") {
      if (allowedRoles.includes("dispatcher") || allowedRoles.includes("dock") || allowedRoles.includes("__driver__")) return next();
      return res.redirect(302, "/");
    }

    if (role === "dock") {
      if (allowedRoles.includes("dock") || allowedRoles.includes("__driver__")) return next();
...
(function(){
  ...
  // DISPATCHER / MANAGEMENT / ADMIN LOGIN
...
app.post("/api/upsert", requireXHR, requireDockStatusAllowed, async (req, res) => {
  ...
  try {
    const trailer = normTrailer(req.body.trailer);
    ...
    if (actor === "dock") {
      const onlyStatus =
        ...
        req.body.carrierType === undefined;
    ...
    await run(
      `INSERT INTO trailers(trailer,direction,status,door,note,dropType,carrierType,updatedAt,doorAt)
       ...
       ON CONFLICT(trailer) DO UPDATE SET
         ...
         doorAt = CASE
           WHEN excluded.door IS NOT NULL AND excluded.door != '' AND excluded.door != trailers.door
           THEN excluded.doorAt
           ELSE trailers.doorAt
         END`,
...
/* ══════════════════════════════════════════
   API — DOOR BLOCKS & DOCK PLATES
══════════════════════════════════════════ */
app.get("/api/doorblocks", async (_req, res) => {
  try { res.json(await loadDoorBlocksObject()); }
...
app.post("/api/doorblock/set", requireXHR, requireRole(["dock","dispatcher","management","admin"]), async (req, res) => {
  try {
    const door = normDoor(req.body.door);
    const note = String(req.body.note || "").slice(0, 120);
    const dNum = Number(door);
    if (!door || !Number.isFinite(dNum) || dNum < 28 || dNum > 42) return res.status(400).send("Invalid door");
    await run(
      `INSERT INTO doorblocks(door,note,setAt) VALUES(?,?,?)
    ...
    console.error("[doorblock/set]", e);
    res.status(500).send("Doorblock set failed");
  }
});
...
app.post("/api/doorblock/clear", requireXHR, requireRole(["dock","dispatcher","management","admin"]), async (req, res) => {
  try {
    ...
    console.error("[doorblock/clear]", e);
    res.status(500).send("Doorblock clear failed");
  }
});

app.get("/api/dockplates", async (_req, res) => {
  try { res.json(await loadDockPlatesObject()); }
...
app.post("/api/dockplates/set", requireXHR, requireRole(["dock","dispatcher","management","admin"]), async (req, res) => {
  try {
    ...
    console.error("[dockplates/set]", e);
    res.status(500).send("Dock plate set failed");
  }
});

/* ══════════════════════════════════════════
   API — DRIVER (OMW / ARRIVE / DROP)
══════════════════════════════════════════ */
app.post("/api/driver/omw", requireXHR, async (req, res) => {
  try {
    const trailer = normTrailer(req.body.trailer);
    const eta     = Number.isFinite(parseInt(req.body.eta)) ? parseInt(req.body.eta) : null;
    ...
    await run(
      `INSERT INTO trailers(trailer,direction,status,door,note,dropType,carrierType,updatedAt,omwAt,omwEta)
       ...
       ON CONFLICT(trailer) DO UPDATE SET
         ...
         carrierType=excluded.carrierType,
         updatedAt=excluded.updatedAt,
         omwAt=excluded.omwAt,
         omwEta=excluded.omwEta`,
    ...
    broadcastPush("🚛 Driver On My Way", `Trailer ${trailer} → Door ${assignedDoor}${eta ? ` · ETA ~${eta} min` : ""}`, { type: "omw", trailer, door: assignedDoor }).catch(() => {});
    ...
    console.error("[omw]", e);
    res.status(500).send("OMW failed");
  }
});

app.get("/api/driver/assignment", async (req, res) => {
  try {
    const trailer = normTrailer(req.query.trailer);
    if (!trailer) return res.status(400).send("Missing trailer");
...
app.get("/api/available-doors", async (req, res) => {
  try {
    const excludeTrailer = req.query.trailer ? normTrailer(req.query.trailer) : null;
    const occupiedSet = await getOccupiedDoorSet(excludeTrailer);
    const plates = await all(`SELECT door, status FROM dockplates`);
    const plateMap = {};
    plates.forEach(p => { plateMap[String(p.door)] = p.status; });
    ...
    for (let d = 28; d <= 42; d++) {
      ...
      const plateStatus = plateMap[ds] || "Unknown";
      if (plateStatus === "Out of Order") continue;
      doors.push({ door: ds, available: !occupiedSet.has(ds), plateStatus });
    }
    res.json({ doors });
  } catch {
    res.status(500).send("Available doors error");
  }
});
...
app.post("/api/driver/arrive", requireXHR, async (req, res) => {
  try {
    ...
    console.error("[arrive]", e);
    res.status(500).send("Arrival failed");
  }
});
...
/* ══════════════════════════════════════════
   CSV EXPORT
══════════════════════════════════════════ */
app.get("/api/export/trailers.csv", requireRole(["dispatcher","management","admin"]), async (_req, res) => {
  try {
...
app.post("/api/confirm-safety", requireXHR, requireDriverAccess, async (req, res) => {
  try {
    ...
    console.error("[confirm-safety]", e);
...
async function fireWebhook(event, data) {
  ...
  try {
    ...
    logEvent("warn", "webhook", `Webhook failed for ${event}`, e.message).catch(() => {});
  }
}
  ...
  .then(() => {
    ...
    async function archiveDeparted() {
      try {
        ...
        logEvent("error","archive","Auto-archive failed", e.message).catch(() => {});
      }
    }
    setInterval(archiveDeparted, 3600 * 1000).unref();
    archiveDeparted();

    // Hourly SQLite backup (CONSISTENT)
    async function backupDb() {
      try {
        ...
        logEvent("error", "backup", "DB backup failed", e.message).catch(() => {});
        console.error("[BACKUP]", e.message);
      }
    }
    setInterval(backupDb, 3600 * 1000).unref();
