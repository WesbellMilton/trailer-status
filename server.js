...
const express=require("express"),http=require("http"),path=require("path"),
  fs=require("fs"),WebSocket=require("ws"),sqlite3=require("sqlite3").verbose(),
  crypto=require("crypto"),zlib=require("zlib");
const app=express();

process.on("uncaughtException",  e=>{ console.error("[CRASH] uncaughtException:",e);  logEvent("error","crash","uncaughtException",String(e?.stack||e)).catch(()=>{}); });
process.on("unhandledRejection", e=>{ console.error("[CRASH] unhandledRejection:",e); logEvent("error","crash","unhandledRejection",String(e?.stack||e)).catch(()=>{}); });
...
// ── Request timeout 30s ──
app.use((req,res,next)=>{
  const t=setTimeout(()=>{ if(!res.headersSent)res.status(503).send("Request timeout"); },30000);
...
function checkLoginRate(ip){
  ...
  if(!e||now>e.resetAt){ e={count:0,resetAt:now+60000}; loginAttempts.set(ip,e); }
  return ++e.count<=15;
}
...
const DB_FILE=process.env.DB_FILE||(()=>{
  ...
  for(const c of["/var/data/wesbell.sqlite","/tmp/wesbell.sqlite"]){
    try{fs.mkdirSync(p.dirname(c),{recursive:true});return c;}catch{}
  }
...
const ENV_PINS={dispatcher:process.env.DISPATCHER_PIN||"",dock:process.env.DOCK_PIN||"",management:process.env.MANAGEMENT_PIN||"",admin:process.env.ADMIN_PIN||""};

function requireXHR(req,res,next){
  if((req.get("X-Requested-With")||"").toLowerCase()!=="xmlhttprequest")return res.status(400).send("Bad request");
  next();
}
...
// ── Caches ──
let _trailersCache=null,_platesCache=null,_blocksCache=null;
const invalidateTrailers=()=>_trailersCache=null;
const invalidatePlates=  ()=>_platesCache=null;
const invalidateBlocks=  ()=>_blocksCache=null;
...
function loadOrGenVapid(){
  try{
    if(fs.existsSync(VAPID_FILE)){ VAPID_KEYS=JSON.parse(fs.readFileSync(VAPID_FILE,"utf8")); console.log("[VAPID] Loaded existing keys"); return; }
  }catch{}
  try{
    const {publicKey,privateKey}=crypto.generateKeyPairSync("ec",{namedCurve:"P-256",publicKeyEncoding:{type:"spki",format:"der"},privateKeyEncoding:{type:"pkcs8",format:"der"}});
...
async function buildVapidJWT(audience){
  ...
  const privKey=crypto.createPrivateKey({
    key:Buffer.concat([Buffer.from("308187020100301306072a8648ce3d020106082a8648ce3d030107046d306b0201010420","hex"),fromb64url(VAPID_KEYS.privateKey),Buffer.from("a144034200","hex"),fromb64url(VAPID_KEYS.publicKey)]),
...
async function encryptPushPayload(plaintext,keys){
  ...
  const clientPub=crypto.createPublicKey({key:Buffer.concat([Buffer.from("3059301306072a8648ce3d020106082a8648ce3d030107034200","hex"),clientPubRaw]),format:"der",type:"spki"});
...
async function sendPush(subscription,payload){
  ...
  return new Promise((resolve,reject)=>{
    const req=https.request(endpoint,{method:"POST",headers:{"Authorization":`vapid t=${jwt},k=${VAPID_KEYS.publicKey}`,"Content-Type":"application/octet-stream","Content-Encoding":"aes128gcm","TTL":"86400","Content-Length":encrypted.length}},res=>{res.resume();resolve(res.statusCode)});
    req.on("error",reject); req.write(encrypted); req.end();
  });
}
...
async function broadcastPush(title,body,data){
  ...
  for(const[endpoint,sub]of pushSubs){
    try{ const s=await sendPush(sub,payload); if(s===410||s===404)dead.push(endpoint); }catch{}
  }
  if(dead.length){
    dead.forEach(ep=>pushSubs.delete(ep));
    await run(`DELETE FROM push_subscriptions WHERE endpoint IN (${dead.map(()=>"?").join(",")})`,dead).catch(()=>{});
  }
}
...
async function initDb(){
  ...
  await run(`CREATE TABLE IF NOT EXISTS audit(id INTEGER PRIMARY KEY AUTOINCREMENT,at INTEGER,actorRole TEXT,action TEXT,entityType TEXT,entityId TEXT,details TEXT,ip TEXT,userAgent TEXT)`);
  await run(`CREATE TABLE IF NOT EXISTS push_subscriptions(endpoint TEXT PRIMARY KEY,subscription TEXT,createdAt INTEGER)`);
  await run(`CREATE TABLE IF NOT EXISTS issue_reports(id INTEGER PRIMARY KEY AUTOINCREMENT,at INTEGER,trailer TEXT,door TEXT,note TEXT,photo_data TEXT,photo_mime TEXT,ip TEXT,userAgent TEXT)`);
  ...
  for(let d=28;d<=42;d++){
    const door=String(d);
    if(!existingPlates.has(door))await run(`INSERT INTO dockplates(door,status,note,updatedAt) VALUES(?,?,?,?)`,[door,"Unknown","",Date.now()]);
  }
  ...
  for(const role of["dispatcher","dock","management","admin"]){
    ...
    else if(envPin){ await setPin(role,envPin); console.log(`[SECURITY] ${role} PIN synced from environment`); }
  }
}

const genTempPin=()=>String(crypto.randomInt(100000,1000000));

function pbkdf2Hash(pin,salt,iter=140000){
  return new Promise((res,rej)=>crypto.pbkdf2(pin,salt,iter,32,"sha256",(e,d)=>e?rej(e):res(d)));
}

async function setPin(role,pin){
  const salt=crypto.randomBytes(16),iter=140000;
  const hash=await pbkdf2Hash(pin,salt,iter);
  await run(`INSERT INTO pins(role,salt,hash,iter) VALUES(?,?,?,?) ON CONFLICT(role) DO UPDATE SET salt=excluded.salt,hash=excluded.hash,iter=excluded.iter`,[role,salt,hash,iter]);
}
...
setInterval(()=>{ const now=Date.now(); for(const[sid,s]of sessions.entries())if(s.exp<now)sessions.delete(sid); },30*60*1000).unref();

function newSession(role){ const sid=crypto.randomBytes(24).toString("hex"); sessions.set(sid,{role,exp:Date.now()+SESSION_TTL_MS}); return sid; }
...
function setSessionCookie(res,sid){
  ...
  res.setHeader("Set-Cookie",`${COOKIE_NAME}=${encodeURIComponent(sid)}; Path=/; HttpOnly; SameSite=Lax${secure?"; Secure":""}`);
}
function clearSessionCookie(res){ res.setHeader("Set-Cookie",`${COOKIE_NAME}=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0`); }
...
function requireIssueAccess(req,res,next){ next(); }
...
function requireDockStatusAllowed(req,res,next){
  ...
  if(s.role==="dock"){
    ...
    if(status&&!["Loading","Dock Ready"].includes(status))return res.status(403).send(`Dock role cannot set status: ${status}`);
    return next();
  }
...
// ── Helpers ──
const ipOf=req=>{ const xf=req.headers["x-forwarded-for"]; return xf?String(xf).split(",")[0].trim():req.socket.remoteAddress||""; };

const MAX_LOGS=500;
async function logEvent(level,context,message,detail=""){
  try{
    await run(`INSERT INTO logs(at,level,context,message,detail) VALUES(?,?,?,?,?)`,[Date.now(),level,context,message,String(detail).slice(0,500)]);
...
async function audit(req,actorRole,action,entityType,entityId,details){
  ...
  await run(`INSERT INTO audit(at,actorRole,action,entityType,entityId,details,ip,userAgent) VALUES(?,?,?,?,?,?,?,?)`,[Date.now(),actorRole||"unknown",action,entityType,entityId,d,ipOf(req),req.headers["user-agent"]||""]);
}
...
const loadConfirmations=(limit=250)=>all(`SELECT at,trailer,door,action,ip,userAgent FROM confirmations ORDER BY at DESC LIMIT ?`,[limit]);
...
async function getOccupiedDoorSet(excludeTrailer=null){
  const occupied=await all(`SELECT door FROM trailers WHERE door IS NOT NULL AND door != '' AND status NOT IN ('Departed','')${excludeTrailer?" AND trailer != ?":""}`,excludeTrailer?[excludeTrailer]:[]);
  ...
  return new Set([...occupied.map(r=>String(r.door)),...blocks.map(r=>String(r.door)),...reserved.map(r=>String(r.door))]);
}
...
const releaseReservation=trailer=>run(`DELETE FROM door_reservations WHERE trailer=?`,[trailer]);

async function cleanupExpiredReservations(){
  const r=await run(`DELETE FROM door_reservations WHERE expiresAt <= ?`,[Date.now()]);
  if(r.changes>0){ console.log(`[reservations] Cleaned up ${r.changes} expired reservation(s)`); await broadcastTrailers(); }
}
...
app.use((req,res,next)=>{
  ...
  res.sendFile(path.join(__dirname,req.path.replace(/\/\.\./g,"")),err=>{if(err&&!res.headersSent)res.status(404).end()});
});
...
function guardPage(allowedRoles){
  return(req,res,next)=>{
    ...
    if(!role){ return allowedRoles.includes("__driver__")?next():res.redirect(302,`/login?from=${encodeURIComponent(req.path)}`); }
    ...
    return res.redirect(302,roleHome(role)||"/");
  };
}
...
.logo,.heading,.sub,.card{animation:fadeUp .3s ease both}
.heading{animation-delay:.06s}.sub{animation-delay:.1s}.card{animation-delay:.14s}
</style></head><body>
...
@media(max-width:768px){
  body{grid-template-columns:1fr;grid-template-rows:auto 1fr;overflow-y:auto;height:auto}
  ...
  .divider,.cal-wrap{display:none}
  .db-footer{flex:0 0 100%;margin-top:8px;padding-top:8px}
  .login-panel{padding:28px 20px max(28px,env(safe-area-inset-bottom,28px))}
...
<div class="dashboard">
  <div class="db-brand"><div class="db-mark">W</div><div><div class="db-name">WESBELL</div><div class="db-sub">DISPATCH SYSTEM</div></div></div>
  <div class="clock-wrap"><span class="clock-time"><span id="ch">--</span><span class="colon">:</span><span id="cm">--</span><span class="clock-secs" id="cs">--</span></span><span class="clock-ampm" id="ca"></span></div>
...
<div class="login-panel">
  <div class="lp-brand"><div class="lp-mark">W</div><div><div class="lp-name">WESBELL</div><div class="lp-sub2">DISPATCH</div></div></div>
...
app.post("/api/upsert",requireXHR,requireDockStatusAllowed,async(req,res)=>{
  ...
  try{
    ...
    if(actor==="dock"){
      const onlyStatus=req.body.status!==undefined&&req.body.direction===undefined&&req.body.door===undefined&&req.body.note===undefined&&req.body.dropType===undefined;
    ...
    await run(`INSERT INTO trailers(trailer,direction,status,door,note,dropType,carrierType,updatedAt,doorAt) VALUES(?,?,?,?,?,?,?,?,?) ON CONFLICT(trailer) DO UPDATE SET direction=excluded.direction,status=excluded.status,door=excluded.door,note=excluded.note,dropType=excluded.dropType,carrierType=excluded.carrierType,updatedAt=excluded.updatedAt,doorAt=COALESCE(trailers.doorAt,excluded.doorAt)`,[trailer,direction,finalStatus,door,note,dropType,carrierType,now,doorAt]);
    ...
    if(finalStatus!==(existing?.status)){
      if(finalStatus==="Ready"){ wsBroadcast("notify",{kind:"ready",trailer,door:door||""}); broadcastPush("🟢 Trailer Ready",`Trailer ${trailer} is ready${door?" at door "+door:""}`,{trailer,door}).catch(()=>{}); fireWebhook("trailer.ready",{trailer,door,actor}); }
...
// ── Dock plates / doorblocks ──
app.get("/api/doorblocks",async(req,res)=>{ try{res.json(await loadDoorBlocksObject())}catch{res.status(500).send("Doorblocks error")} });
...
app.get("/api/dockplates",async(req,res)=>{ try{res.json(await loadDockPlatesObject())}catch{res.status(500).send("Plates error")} });
...
app.post("/api/dockplates/set",requireXHR,requireRole(["dock","dispatcher","management","admin"]),async(req,res)=>{
  try{
    ...
    await run(`INSERT INTO dockplates(door,status,note,updatedAt) VALUES(?,?,?,?) ON CONFLICT(door) DO UPDATE SET status=excluded.status,note=excluded.note,updatedAt=excluded.updatedAt`,[door,status,note,Date.now()]);
...
app.post("/api/driver/omw",requireXHR,requireDriverRate,async(req,res)=>{
  try{
    ...
    if(existing&&ACTIVE.includes(existing.status))return res.json({ok:true,door:existing.door||"",alreadyActive:true,status:existing.status});
    ...
    const now=Date.now();
    await run(`INSERT INTO trailers(trailer,direction,status,door,note,dropType,carrierType,updatedAt,omwAt,omwEta) VALUES(?,?,?,?,?,?,?,?,?,?) ON CONFLICT(trailer) DO UPDATE SET direction=excluded.direction,status=excluded.status,door=excluded.door,note=excluded.note,dropType=excluded.dropType,carrierType=excluded.carrierType,updatedAt=excluded.updatedAt,omwAt=excluded.omwAt,omwEta=excluded.omwEta`,[trailer,"Inbound","Incoming",assignedDoor,note,"Loaded","Wesbell",now,now,eta]);
...
app.post("/api/driver/arrive",requireXHR,requireDriverRate,async(req,res)=>{
  try{
    ...
    const existing=await get(`SELECT * FROM trailers WHERE trailer=?`,[trailer]);
    const ACTIVE=["Incoming","Dropped","Loading","Dock Ready","Ready"];
    // If already on the board WITH a door, return the existing door immediately
    ...
    // If already on board WITHOUT a door (e.g. outside carrier quick-drop), assign one now
    const assignedDoor=await pickBestDoor(trailer);
    ...
    const now=Date.now();
    // Preserve existing note/dropType if updating an already-Dropped record
    const useDropType=existing?.dropType||dropType;
    const useDirection=existing?.direction||direction;
    await run(`INSERT INTO trailers(trailer,direction,status,door,note,dropType,carrierType,updatedAt,doorAt) VALUES(?,?,?,?,?,?,?,?,?) ON CONFLICT(trailer) DO UPDATE SET direction=excluded.direction,status=excluded.status,door=excluded.door,dropType=excluded.dropType,carrierType=excluded.carrierType,updatedAt=excluded.updatedAt,doorAt=excluded.doorAt`,[trailer,useDirection,"Incoming",assignedDoor,existing?.note||"",useDropType,carrierType,now,now]);
    await releaseReservation(trailer); // clear temp reservation now that row owns the door
...
app.post("/api/driver/drop",requireXHR,requireDriverRate,requireDriverAccess,async(req,res)=>{
  try{
    ...
    if(existing&&ACTIVE.includes(existing.status)&&!req.body.force){
      return res.status(409).json({duplicate:true,trailer,currentStatus:existing.status,currentDoor:existing.door||null,message:`Trailer ${trailer} is already on the board (${existing.status}${existing.door?" at door "+existing.door:""}). Submit again to overwrite.`});
    }
    // Outside carriers dropping without a door should appear as "Dropped" so dispatchers
    // know a door assignment is needed. Wesbell drops with no door are "Incoming" (they'll arrive shortly).
    const dropStatus=door?"Incoming":(carrierType==="Outside"?"Dropped":"Incoming");
    await run(`INSERT INTO trailers(trailer,direction,status,door,note,dropType,carrierType,updatedAt) VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(trailer) DO UPDATE SET direction=excluded.direction,status=excluded.status,door=excluded.door,dropType=excluded.dropType,carrierType=excluded.carrierType,updatedAt=excluded.updatedAt`,[trailer,existing?.direction||"Inbound",dropStatus,door||"",existing?.note||"",dropType,carrierType,now]);
    await releaseReservation(trailer);
    await audit(req,"driver","driver_drop","trailer",trailer,{door:door||"",dropType,carrierType});
    if(dropStatus==="Dropped")broadcastPush("📦 Outside Carrier Drop",`Trailer ${trailer} dropped to yard — needs door assignment`,{trailer}).catch(()=>{});
    await broadcastTrailers();
...
app.post("/api/crossdock/pickup",requireXHR,requireDriverRate,requireDriverAccess,async(req,res)=>{
  try{
    ...
    const existing=await get(`SELECT * FROM trailers WHERE trailer=?`,[trailer]);
    await run(`INSERT INTO trailers(trailer,direction,status,door,note,dropType,updatedAt) VALUES(?,?,?,?,?,?,?) ON CONFLICT(trailer) DO UPDATE SET door=excluded.door,updatedAt=excluded.updatedAt`,[trailer,existing?.direction||"Cross Dock",existing?.status||"Ready",door,existing?.note||"",existing?.dropType||"",Date.now()]);
...
app.post("/api/crossdock/offload",requireXHR,requireDriverRate,requireDriverAccess,async(req,res)=>{
  try{
    ...
    if(existing&&ACTIVE.includes(existing.status)&&existing.door&&existing.door!==door&&!req.body.force){
      return res.status(409).json({duplicate:true,trailer,currentStatus:existing.status,currentDoor:existing.door,message:`Trailer ${trailer} is already active at door ${existing.door} (${existing.status}). Submit again to overwrite.`});
    }
    await run(`INSERT INTO trailers(trailer,direction,status,door,note,dropType,updatedAt) VALUES(?,?,?,?,?,?,?) ON CONFLICT(trailer) DO UPDATE SET door=excluded.door,status=excluded.status,dropType=excluded.dropType,updatedAt=excluded.updatedAt`,[trailer,existing?.direction||"Cross Dock","Dropped",door,existing?.note||"","Loaded",Date.now()]);
...
app.post("/api/push/subscribe",requireXHR,async(req,res)=>{
  try{
    ...
    await run(`INSERT INTO push_subscriptions(endpoint,subscription,createdAt) VALUES(?,?,?) ON CONFLICT(endpoint) DO UPDATE SET subscription=excluded.subscription`,[sub.endpoint,JSON.stringify(sub),Date.now()]);
    res.json({ok:true});
...
app.post("/api/push/unsubscribe",requireXHR,async(req,res)=>{
  try{
    ...
    if(endpoint){ pushSubs.delete(endpoint); await run(`DELETE FROM push_subscriptions WHERE endpoint=?`,[endpoint]); }
    res.json({ok:true});
...
app.get("/api/shift-summary",requireRole(["dispatcher","management","admin"]),async(req,res)=>{
  try{
    ...
    const byStatus={};
    events.filter(e=>e.action==="trailer_status_set").forEach(e=>{ try{const dd=JSON.parse(e.details||"{}");byStatus[dd.status]=(byStatus[dd.status]||0)+1;}catch{} });
...
app.get("/health",async(req,res)=>{
  try{
    ...
    res.json({status:"ok",version:APP_VERSION,uptime:Math.floor(process.uptime()),db:DB_FILE,memory:{rss:Math.round(mem.rss/1024/1024)+"MB",heap:Math.round(mem.heapUsed/1024/1024)+"MB"},wsClients:wss?.clients?.size||0,sessions:sessions.size});
  }catch(e){res.status(503).json({status:"error",error:e.message})}
});

// ── Logs / CSV / Audit / Webhook ──
app.get("/api/logs",requireRole(["admin"]),async(req,res)=>{ try{res.json(await all(`SELECT * FROM logs ORDER BY at DESC LIMIT 200`))}catch{res.status(500).send("Failed")} });
...
app.get("/api/export/audit.csv",requireRole(["management","admin"]),async(req,res)=>{
  try{
    ...
    const rows=await all(`SELECT * FROM audit WHERE at > ? ORDER BY at DESC`,[Date.now()-hours*3600000]);
...
async function fireWebhook(event,data){
  ...
  try{
    ...
    await new Promise((resolve,reject)=>{
      const req=mod.request(WEBHOOK_URL,{method:"POST",headers:{"Content-Type":"application/json","Content-Length":Buffer.byteLength(body)}},res=>{res.resume();resolve()});
      req.on("error",reject); req.setTimeout(5000,()=>req.destroy()); req.write(body); req.end();
    });
...
app.get("/api/audit",requireRole(["dispatcher","management","admin"]),async(req,res)=>{
  try{
    ...
    const rows=await all(`SELECT at,actorRole,action,entityType,entityId,details,ip,userAgent FROM audit ORDER BY at DESC LIMIT ?`,[limit]);
    res.json(rows.map(r=>{ let details={};try{details=r.details?JSON.parse(r.details):{};}catch{} return{...r,details}; }));
...
app.post("/api/report-issue",requireXHR,requireIssueAccess,async(req,res)=>{
  ...
  try{
    ...
    const at=Date.now();
    const result=await run(`INSERT INTO issue_reports(at,trailer,door,note,photo_data,photo_mime,ip,userAgent) VALUES(?,?,?,?,?,?,?,?)`,[at,trailer,door,note,photoData||null,photoMime||null,ipOf(req),req.headers["user-agent"]||""]);
  ...
  .then(()=>{
    ...
    async function archiveDeparted(){
      try{
        ...
        if(r.changes>0){ invalidateTrailers(); await broadcastTrailers(); await logEvent("info","archive",`Auto-archived ${r.changes} departed trailers`); console.log(`[ARCHIVE] Removed ${r.changes} old departed trailers`); }
      }catch(e){logEvent("error","archive","Auto-archive failed",e.message)}
    }
    setInterval(archiveDeparted,3600*1000);
    archiveDeparted();
    ...
    setInterval(backupDb,3600*1000);
    ...
    async function gracefulShutdown(signal){
      ...
      server.close(()=>{ db.close(()=>{ console.log("[SHUTDOWN] Complete"); process.exit(0); }); });
    ...
    setInterval(()=>{
      for(const client of wss.clients)
        if(client.readyState===WebSocket.CLOSING||client.readyState===WebSocket.CLOSED)try{client.terminate()}catch{}
    },30000);
  })
