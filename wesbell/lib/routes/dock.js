...
router.get('/api/doorblocks', async (req, res) => {
  try { res.json(await loadDoorBlocksObject()); } catch { res.status(500).send('Doorblocks error'); }
});
...
router.post('/api/doorblock/set', requireXHR, requireRole(['dock', 'dispatcher', 'management', 'admin']), async (req, res) => {
  try {
    ...
    await run(`INSERT INTO doorblocks(door,note,setAt) VALUES(?,?,?)
...
router.get('/api/dockplates', async (req, res) => {
  try { res.json(await loadDockPlatesObject()); } catch { res.status(500).send('Plates error'); }
});
