...
const { loadConfirmations } = require('./cache');

let _app = null;   // set via init()
let _wss = null;
let _server = null;

// FIX: was calling raw loaders (loadTrailersObject etc.) which bypass the cache,
// causing a fresh DB query on every new connection. Use the cached getters instead.
async function sendInitialState(ws, safeSend) {
  const locId = ws.locationId || 1;
  const cache = require('./cache');
  try { safeSend(JSON.stringify({ type: 'state',         payload: await cache.getTrailersCache(locId) })); } catch {}
  try { safeSend(JSON.stringify({ type: 'dockplates',    payload: await cache.getPlatesCache(locId) })); } catch {}
  try { safeSend(JSON.stringify({ type: 'doorblocks',    payload: await cache.getBlocksCache(locId) })); } catch {}
...
// FIX: was using wsBroadcast (all clients) — must be location-scoped like broadcastTrailers.
// Also mirrors the debounce + invalidate pattern for consistency.
async function broadcastPlates(locationId = null) {
  try {
    const cache = require('./cache');
    if (locationId) {
      cache.invalidatePlates(locationId);
      const data = await cache.getPlatesCache(locationId);
      wsBroadcastToLocation(locationId, 'dockplates', data);
    } else {
      cache.invalidatePlates();
      wsBroadcast('dockplates', await cache.getPlatesCache());
    }
  } catch (e) { console.error('[WS] broadcastPlates:', e.message); }
}

async function broadcastBlocks(locationId = null) {
  try {
    const cache = require('./cache');
    if (locationId) {
      cache.invalidateBlocks(locationId);
      const data = await cache.getBlocksCache(locationId);
      wsBroadcastToLocation(locationId, 'doorblocks', data);
    } else {
      cache.invalidateBlocks();
      wsBroadcast('doorblocks', await cache.getBlocksCache());
    }
